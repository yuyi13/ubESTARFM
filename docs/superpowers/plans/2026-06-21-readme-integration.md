# Integrated README Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Restore the published README's scientific narrative and supporting material while documenting the maintained R/Python packages, reusable-model workflow, batch prediction, and current repository layout.

**Architecture:** Keep `README.md` as a self-contained landing page with the published scientific context first and the maintained package workflow immediately after it. Preserve historical provenance by linking archived files at their current descriptive paths, while routing implementation details to the existing tutorials, migration notes, examples, and benchmarks.

**Tech Stack:** GitHub-flavored Markdown, R 4.3+, terra, Rcpp, Python 3.12+, NumPy, rasterio, Numba, pytest.

---

### Task 1: Restore the published README structure and scientific narrative

**Files:**
- Modify: `README.md`

- [ ] **Step 1: Replace the current abbreviated opening with the integrated title, badges, contents, overview, background, and repository structure**

Use this exact opening in `README.md`:
...
- [ ] **Step 2: Check the restored opening for stale implementation claims**
...
assert "The ubESTARFM algorithm is written in R." not in text
assert "install.packages('raster')" not in text
PY
```

Expected: exit status 0 with no output.

- [ ] **Step 3: Commit the restored narrative and structure**

```bash
git add README.md
git commit -m "docs: restore published README narrative"
```

Expected: one commit modifying only `README.md`.

### Task 2: Integrate the maintained R/Python workflow and archived study material

**Files:**
- Modify: `README.md`

- [ ] **Step 1: Append installation and runnable R/Python usage**

Append this exact content after the repository-structure section:
...
- [ ] **Step 2: Append the study-processing, data, documentation, citation, conference, and references sections**
...
- [ ] **Step 3: Verify that the documented API names and current paths are present**
...
required_paths = [
    ...
    "legacy/examples/",
    "inst/extdata/",
    "examples/outputs/legacy_visualisation.png",
    "legacy/paper-processing/",
    "data/ozflux/",
]
assert all(item in text for item in required_api)
assert all(item in text for item in required_paths)
PY
```

Expected: exit status 0 with no output.

- [ ] **Step 4: Commit the integrated package and study documentation**

```bash
git add README.md
git commit -m "docs: integrate modern package workflows"
```

Expected: one commit modifying only `README.md`.

### Task 3: Validate the complete README

**Files:**
- Verify: `README.md`

- [ ] **Step 1: Validate every Contents entry against a README heading**
...
contents = text.split("## Overview", 1)[0]
targets = re.findall(r"\]\(#([^)]+)\)", contents)
missing = sorted(set(targets) - anchors)
assert not missing, f"Missing heading anchors: {missing}"
PY
```

Expected: exit status 0 with no output.

- [ ] **Step 2: Validate every local Markdown link and image path**
...
assert not missing, f"Missing local paths: {sorted(set(missing))}"
PY
```

Expected: exit status 0 with no output.

- [ ] **Step 3: Smoke-test the maintained examples referenced by the README**

Run:

```bash
python3 -m pytest \
  tests/layout/test_layout.py \
